...
export default function PatientOnboarding() {
  ...
  const [accuracyNotice, setAccuracyNotice] = useState<number | null>(null);
  ...
  const handleAutoDetectAddress = async () => {
    ...
    try {
      const { lat, lng, accuracy } = await getBrowserPosition(25, 8000);
      setAccuracyNotice(Math.round(accuracy));

      const result = await fetchNeighborhoodFromCoords(lat, lng, accuracy);
      ...
      if (result && result.formattedAddress) {
        ...
        setError('Could not identify your exact locality. Please type your address manually.');
      }
    } catch (err: any) {
      ...
      if (err.code === 1) {
        ...
        setError('Unable to detect GPS position. Please enter your address manually.');
      }
    } finally {
  ...
  return (
    <div className="min-h-[100dvh] w-screen bg-gradient-to-br from-slate-50 via-slate-100 to-emerald-50/40 flex items-center justify-center p-3 sm:p-6 md:p-8 font-sans text-slate-800 antialiased overflow-y-auto selection:bg-emerald-100 selection:text-emerald-900">
      <div className="w-full max-w-3xl">
        <div className="bg-white/95 backdrop-blur-md px-5 py-6 sm:p-8 md:p-9 rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-200/90 transition-all">
          ...
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-800 transition-colors group cursor-pointer p-1"
            >
            ...
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 rounded-full border border-emerald-200/60">
          ...
          <div className="mb-4">
            ...
            <p className="text-xs sm:text-sm font-normal text-slate-500 mt-1 leading-relaxed">
              Provide your medical identifiers and residential locality for care records and emergency routing.
            </p>
          </div>

          {/* Error Alert */}
          {error && (
            <div className="mb-4 p-3 rounded-xl sm:rounded-2xl bg-rose-50/90 border border-rose-200 text-rose-800 text-xs sm:text-sm font-medium flex items-start gap-2.5 shadow-xs">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-slate-50/70 border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-600 focus:bg-white text-base sm:text-sm font-normal transition-all shadow-xs min-h-[44px]"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-slate-50/70 border border-slate-300 rounded-xl text-slate-900 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-600 focus:bg-white text-base sm:text-sm font-normal transition-all shadow-xs min-h-[44px] cursor-pointer"
                  />
                </div>
              </div>
              ...
              <div className="min-w-0">
                ...
                <div className="relative group min-w-0">
                  ...
                  <select
                    ...
                    className={`w-full appearance-none pl-10 pr-10 py-2.5 bg-slate-50/70 border border-slate-300 rounded-xl text-base sm:text-sm transition-all shadow-xs focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-600 focus:bg-white cursor-pointer truncate min-h-[44px] ${
                ...
                <div className="relative group min-w-0">
                  ...
                  >
                    ...
                    <optgroup label="Common Groups" className="text-slate-800 font-semibold">
                      <option value="O_POS" className="text-slate-900 font-normal">O+ (Universal Donor)</option>
              ...
              <div>
                ...
                <div className="relative flex items-center group">
                  ...
                  <input
                    ...
                    className="w-full pl-20 pr-4 py-2.5 bg-slate-50/70 border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-600 focus:bg-white text-base sm:text-sm font-normal transition-all shadow-xs min-h-[44px]"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative flex items-center group">
                  ...
                  <input
                    ...
                    className="w-full pl-20 pr-4 py-2.5 bg-slate-50/70 border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500 focus:bg-white text-base sm:text-sm font-normal transition-all shadow-xs min-h-[44px]"
                  />
                </div>
              </div>
              ...
              <div className="md:col-span-2">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 mb-1.5">
                  ...
                  <button
                    ...
                    className="self-start sm:self-auto inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-800 hover:text-emerald-950 bg-emerald-50 hover:bg-emerald-100/80 active:bg-emerald-200/80 border border-emerald-300/80 px-3 py-1.5 rounded-lg transition-all cursor-pointer disabled:opacity-60 shadow-2xs min-h-[36px]"
                  >
                    {isDetectingLocation ? (
                      <>
                        <span className="w-3.5 h-3.5 border-2 border-emerald-700 border-t-transparent rounded-full animate-spin" />
                        <span>Detecting Locality...</span>
                      </>
                    ) : (
                      <>
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2.5 bg-slate-50/70 border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-600 focus:bg-white text-base sm:text-sm font-normal resize-none transition-all shadow-xs"
                />

                {detectedPincodeInfo && (
                  <div className="mt-2 flex flex-wrap items-center justify-between gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-50/80 border border-emerald-200 text-xs font-medium text-emerald-800">
                    <div className="flex items-center gap-1.5 truncate">
                      <span className="font-bold text-emerald-700 shrink-0">✓ Area:</span>
                      <span className="truncate">{detectedPincodeInfo}</span>
                    </div>
                    {accuracyNotice && (
                      <span className="text-[10px] sm:text-[11px] text-emerald-700 bg-emerald-100/70 px-2 py-0.5 rounded-full font-semibold">
                        ±{accuracyNotice}m precision
                      </span>
                    )}
                  </div>
                )}
            ...
            <div className="pt-2 sm:pt-3">
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-3.5 bg-emerald-700 hover:bg-emerald-800 active:scale-[0.99] text-white font-bold rounded-xl focus:outline-none focus:ring-4 focus:ring-emerald-600/20 transition-all disabled:opacity-70 text-sm sm:text-base cursor-pointer shadow-md shadow-emerald-700/20 flex items-center justify-center gap-2 min-h-[48px]"
              >
                {isLoading ? (
                  <>
